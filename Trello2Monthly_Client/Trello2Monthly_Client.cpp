...
private:
	...
	std::optional<bool> get_pdf()
	{
		...
		temp_builder.append_query(U("name"), filename_);
			...
			.then([=](http_response response)
		{
			...
			std::wcout << "Succeed in POSTING the tex file to server.";
	...
	bool send_tex_file()
	{
		...
		pplx::task<bool> put_task = client_.request(methods::PUT, U(""), result)
			// Handle response headers arriving.
			.then([=](http_response response)
		{
			if (response.status_code() != status_codes::OK)
